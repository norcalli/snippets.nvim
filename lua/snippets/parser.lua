...
--   == { 'for '; 1; ' '; 2; ' in ipairs('...
local function parse_snippet(body, patterns, snippet_name)
	local R = {}
	...
	for LOOP_IDX = 1, 10000000 do
		...
		local left_pos, var_id, placeholder, right_pos, kind
		if #next_value == 3 then
			left_pos, right_pos, var_id = unpack(next_value)
		elseif #next_value == 4 then
			left_pos, right_pos, kind, placeholder = unpack(next_value)
			verbatim_index = verbatim_index + 1
			var_id = -verbatim_index
		else
			assert(#next_value == 5, #next_value)
			left_pos, right_pos, var_id, kind, placeholder = unpack(next_value)
		end
		if not var_id then
		...
		if variables[var_id] then
			if placeholder ~= nil and variables[var_id].placeholder ~= nil then
				return nil, "Multiple placeholders defined for variable $"..var_id
			end
		else
			if kind == '=' then
				-- TODO(ashkan): pass name in here for the chunkname?
				local chunk_name = ("snippets[name=%q id=%d]"):format(snippet_name or "?", var_id)
				placeholder = assert(loadstring("return "..placeholder, chunk_name))
			end
			variables[var_id] = {
